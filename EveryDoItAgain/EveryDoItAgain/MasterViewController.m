...
- (void)insertNewObject:(id)sender {
    ...
    __block UITextField * toDoTitleTextField;
    __block UITextField * toDoDescriptionTextField;
    //int_64, 32, 16 always going to be NSNumber which provides the best precision, not going to need more precision than 4billion for 32 b integer type
    __block UITextField * toDoPriorityTextField;


    UIAlertController *addToDoAlert = [UIAlertController alertControllerWithTitle:@"Add a To do:" message:nil preferredStyle:UIAlertControllerStyleAlert];
    
    [addToDoAlert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"Title";
        toDoTitleTextField = textField;
    }];
    [addToDoAlert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"Desciption";
        toDoDescriptionTextField = textField;
    }];
    [addToDoAlert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"priority number";
        toDoPriorityTextField = textField;
    }];
    
    [addToDoAlert addAction:[UIAlertAction actionWithTitle:@"Add" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        
        NSString *todoTitle = toDoTitleTextField.text;
        NSString *todoDescription = toDoDescriptionTextField.text;
        NSNumber *priorityNumber = [[[NSNumberFormatter alloc] init] numberFromString:toDoPriorityTextField.text];
        ...
        todo.desc = todoDescription;
        todo.priorityNum = [priorityNumber integerValue];
    ...
    [self presentViewController:addToDoAlert animated:NO completion:nil];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        //NSManagedObjectContext formerly was NSMutableArray of ToDos
